...
abstract class Sabel_Db_Model extends Sabel_Object
{
   ...
   * @return Sabel_Db_Model
   */
  public function selectOneForUpdate($arg1 = null, $arg2 = null)
  {
    $result = null;
    if ($this->hasMethod("beforeSelect")) {
      $result = $this->beforeSelect("selectOneForUpdate");
    }
    ...
    if ($result === null) {
      ...
      $this->setCondition($arg1, $arg2);
      $model = MODEL($this->modelName);
      $this->_doSelectOne($model, true);
      $result = $model;
    }
    
    if ($this->hasMethod("afterSelect")) {
      $afterResult = $this->afterSelect($result, "selectOneForUpdate");
      if ($afterResult !== null) $result = $afterResult;
    }
   ...
   * @return void
   */
  protected function _doSelectOne(Sabel_Db_Model $model, $forUpdate = false)
  {
    $stmt = $this->prepareStatement(Sabel_Db_Statement::SELECT);
    if ($forUpdate) {
        $rows = $this->prepareSelect($stmt)->forUpdate(true)->execute();
