...
class Acl_User extends Sabel_Object
{
    ...
    protected $isRedirectedToEnterPage = false;
    ...
    public function isRedirectedToEnterPage()
    {
        return $this->isRedirectedToEnterPage;
    }
    ...
    public function enter($redirectTo, $backTo = null)
    {
        if ($response = Sabel_Context::getResponse()) {
            ...
            $this->isRedirectedToEnterPage = true;
        } else {
    ...
    public function hasRole($role/*, ... */)
    {
        foreach (func_get_args() as $role) {
            if (in_array($role, $this->values["roles"], true)) {
                return true;
            }
        }
        return false;
