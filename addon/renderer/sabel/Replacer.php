...
class Renderer_Sabel_Replacer extends Sabel_Object
{
    ...
    protected function simpleReplace($contents)
    {
        ...
        return str_replace(array_keys($patterns), array_values($patterns), $contents);
    }
    ...
    protected function foreach_replace($element)
    {
        ...
        if (count($params) === 2) {
            $fmt = "<? foreach ((is_empty(%s)) ? array() : %s as %s) : ?>";
            return sprintf($fmt, $params["from"], $params["from"], $params["value"]);
        } else {
            $fmt = "<? foreach ((is_empty(%s)) ? array() : %s as %s => %s) : ?>";
            return sprintf($fmt, $params["from"], $params["from"], $params["key"], $params["value"]);
        }
    }
    ...
    protected function partial_replace($element)
    {
        ...
        if (($assign = $element->assign) === null) {
            ...
            if (preg_match_all("/'.*[^\\\\]'/U", $assign, $matches) > 0) {
                foreach ($matches[0] as $match) {
                    $assign = str_replace($match, str_replace(",", "__%CMM%__", $match), $assign);
                }
            }
            ...
            $retval = str_replace("__%CMM%__", ",", $retval);
        }

        return $retval;
    }

    protected function block_replace($element)
    {
        if (($name = $element->name) === null) {
            $message = __METHOD__ . "() block name is null.";
            throw new Sabel_Exception_Runtime($message);
        }

        $format = '<?php echo $this->block("%s", array(%s)) ?>';
        $params = array();
        foreach ($element->getAttributes() as $key => $value) {
            if (strpos($key, 'var-') === 0) {
                $key = substr($key, 4);
        ...
        $html = sprintf($format, $name, $params);
    ...
    protected function timer_replace($element)
    {
        $start = trim($element->start);
        $end   = trim($element->end);

        if (preg_match('@^[0-9:]+$@', $start) === 1) {
            $format = 'His';
        } elseif (preg_match('@^[0-9]{4}[-/][0-9]{1,2}[-/][0-9]{1,2}$@', $start) === 1) {
            $format = 'Ymd';
        } else {
            $format = 'YmdHis';
        }

        $fmt  = '<?php if (%s) : ?>' . PHP_EOL;
        $expr = sprintf(
            '%2$d <= %1$d && %1$d < %3$d',
            date($format), date($format, strtotime($start)), date($format, strtotime($end))
        );
