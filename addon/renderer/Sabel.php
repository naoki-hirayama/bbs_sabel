...
class Renderer_Sabel extends Sabel_View_Renderer
{
  ...
  public function block($name, $request_parameters = array())
  {
    @list($name, $param) = explode('/', $name);

    $bus = Sabel_Context::getContext()->getBus();
    $internal = new Sabel_Request_Internal();
    $internal->values($request_parameters);
    $internal->method(Sabel_Request::GET);
    $internal->request(uri(sprintf('n: index, c: block, a: %s, param: %s', $name, $param)));
  ...
  protected function makeCompileFile($template, $compiledPath)
  {
    ...
    $fs->mkfile($compiledPath, 0777)->write($template)->save();
  }
